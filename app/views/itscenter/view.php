...
<div class="prdt">
    <div class="container">
        ...
        <?php if(!empty($find->top_content)): $alt = "Шина размером {$find->value}, установленная на колесной технике"; ?>
        ...
        <div class="prdt-top">
            <div class="col-md-12">
                    ...
                    <?php if(!empty($relatedSizes)): ?>
                        <div class="related-sizes">
                            ...
                            <?php foreach($relatedSizes as $r): ?>
                                <a class="related-sizes__item"
                                href="<?=h(\app\services\filters\FilterUrlHelper::buildBestCategoryFilterPath((int)$r['id'], (string)$r['alias'], (string)$r['url_params']));?>">
                                <?=h($r['value']);?>
                                </a>
                            <?php endforeach; ?>
                            </div>
                ...
                <?php if(!empty($products)): ?>
					...
					<?php
						// 1) Ручной текст — приоритетный
						if (!empty($find->content)) {
							echo $find->content;

						// 2) Если ручного нет — используем InSEO
						} elseif (!empty($inseo->content)) {
